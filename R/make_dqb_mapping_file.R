...
mapping_file <- as.data.frame(readxl::read_xlsx("documents/dqb_sample_identifiers.xlsx", sheet = 1)[,1:2])
...
names <- as.data.frame(readxl::read_xlsx("documents/dqb_sample_identifiers.xlsx", sheet = 2))

## match the order of rows
names <- names[match(mapping_id, names$ID),]

# combine files
dqb_mapping_file <- cbind(names,
                          mapping_file,
...
save(dqb_mapping_file, file = "miseq_reads/DQB-Pool/RData/dqb_mapping_file.RData")
